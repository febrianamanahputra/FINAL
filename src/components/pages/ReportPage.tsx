    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 max-w-[480px] mx-auto w-full">
        <div className="flex items-center justify-between mb-2">
          <div className="text-[9px] text-black/35 uppercase tracking-[1.5px]">Baris Laporan</div>
          ...
          {barisList.map((baris, idx) => (
            <div key={baris.id} className="flex items-center gap-2 border-b-[1.5px] border-primary py-1">
              <textarea
                ...
                className="flex-1 bg-transparent border-none outline-none text-[13px] text-[#1a1a1a] resize-none h-10 leading-relaxed py-2 placeholder:text-black/20"
                rows={1}
              />
              <div className="flex items-center gap-1 shrink-0">
                <button
                  onClick={() => { setActiveBarisId(baris.id); setIsAreaPickerOpen(true); }}
                  className={`p-1 flex items-center justify-center ${baris.area ? 'text-[#4a8800]' : 'text-black/45'}`}
                >
                  <MapPin size={14} strokeWidth={baris.area ? 2 : 1.5} />
                </button>
                <label className="p-1 flex items-center justify-center text-black/70 opacity-70 hover:opacity-100 cursor-pointer">
                ...
                <button onClick={() => handleDeleteBaris(baris.id)} className="p-1 flex items-center justify-center text-black/25 hover:text-red-600 transition-colors">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
      ...
      <Overlay isOpen={isAreaPickerOpen} onClose={() => setIsAreaPickerOpen(false)} title="Pilih Area">
        <div className="flex flex-col gap-1.5 pb-6">
          {locData.areas.length === 0 ? (
            <div className="text-xs text-black/40 text-center py-4">Belum ada area.<br/>Tambah di Kelola Area dulu.</div>
          ) : (
            locData.areas.map((a, i) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-3.5 py-2.5 rounded-lg text-xs transition-colors ${isSelected ? 'bg-primary text-primary-text font-medium' : 'bg-black/5 text-[#1a1a1a]'}`}
                >
